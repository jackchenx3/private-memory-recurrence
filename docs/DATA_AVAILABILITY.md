...
The public release includes complete block-level numerical grids, frozen bootstrap indices, per-group endpoint-fate counts, configurations and seed rosters for studies 045–058. `scripts/reproduce_statistics.py` recomputes all reported means and intervals, including unfavorable and unresolved contrasts. `scripts/rebuild_figures.py` regenerates the fifteen figures from these summaries.
