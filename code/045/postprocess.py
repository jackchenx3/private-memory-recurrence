...
def arms(v,a,g,pol,ms):
 abundances=('RARE','HALF') if a=='RARE_MINUS_HALF' else (a,);regs=('RECUR','IID') if g=='RECUR_MINUS_IID' else (g,);ps=pol.split('-');return {ab+'|'+r+'|'+p:{m:v[ab+'|'+r+'|'+p+'|'+m] for m in ms} for ab in abundances for r in regs for p in ps}
def main():
 ...
 assert n==2304 and len(groups)==192
 ...
 assert len(counts)==len(blocks)==780
 ...
 bounds={g:dict(event_blocks=len(bs),block_event_indicators=[int(b in bs) for b in range(24)],independent_units=24,continuations_per_block=8,zero_event_bound_applicable=len(bs)==0,upper95=1-.05**(1/24.) if not bs else None,unit='probability a fresh eight-continuation block has any endpoint-surviving path',assumption='24 independent identically designed blocks',not_per_lineage_probability=True,not_mean_frequency_interval=True) for g,bs in surviving_blocks.items()};assert len(bounds)==12;save('RARE_BLOCK_EVENTS.json',bounds);save('ALL_PATH_EVENT_SUMMARIES.json',event_paths)
 oldbounds=json.loads((P.parent/'single_carrier_introduction_v1/RARE_BLOCK_EVENTS.json').read_text())
 for k,q in oldbounds.items():assert bounds[k]==q
 save('OLD_BLOCK_EVENTS_CHECK.json',dict(status='PASS',exact_old_groups=8,new_novel_groups=4))
 ratios={g:dict(ratio=q['mean']/values[g[:-3]+'F0']['mean'],ci95=[x/values[g[:-3]+'F0']['mean'] for x in q['ci95']],same_evidence=True) for g,q in values.items() if g.endswith('|F40') and g.split('|')[0] in ('RARE','HALF') and g.split('|')[1] in ('IID','RECUR') and '-' not in g.split('|')[2]};assert len(ratios)==12;save('ENDPOINT_INITIAL_RATIOS.json',ratios)
