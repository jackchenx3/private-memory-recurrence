...
from accepted_transfer_model import Policy,step,frequencies,utilities,to_uniform
...
def stage(targets,draws,pop,cost,fresh,survival,absolute_start=0,boundary_previous_target=None,*,objective):
 assert len(targets)==40 and absolute_start in (0,40) and len(fresh)>=absolute_start+40
 ev=Evaluator(objective);ev.target=targets[0];engine=Policy(cost,pop,fresh);engine.completed=absolute_start;initialscores=[ev.score(q,i) for i,q in enumerate(pop)];iq=ev.records;fs=frequencies(pop);raw=[sum(initialscores)/1024.];pen=[raw[0]+cost*(fs['H']+fs['F'])/32.];freq={r:[x] for r,x in fs.items()};pops=[list(pop)];steps=[];truth=[]
 for g in range(40):
