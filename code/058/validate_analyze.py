"""058 all-new saved-output aggregation; prospective block0 raw audit; no old cohort inputs."""
...
def main():
 ...
 assert nnew==4608 and nold==0 and len(data)==192
 ...
 with (P/'PAIRED_METRICS.jsonl').open('w') as out,(P/'ORIENTATION_METRICS.jsonl').open('w') as oo,(P/'NEGATIVE_OUTCOMES.jsonl').open('w') as no,(P/'TRANSMISSION_ACCURACY_DISAGREEMENTS.jsonl').open('w') as do:
  ...
  for b,cols in sorted(blockcols.items()):
   assert len(cols)==114 and all(len(v)==8 for v in cols.values());blocks.append(dict(block=b,values={k:sum(v)/8 for k,v in cols.items()}))
 save('BLOCK_SUMMARIES.json',blocks);idx=read('BOOTSTRAP_INDICES.json');assert len(idx)==2000 and all(len(x)==24 and all(type(i)==int and 0<=i<24 for i in x) for x in idx)
 estimates={}
 for k in read('METRIC_CATALOG.json')['keys']:
  xs=[b['values'][k] for b in blocks];mean,ci=interval(xs,idx);cl='positive' if ci[0]>0 else 'negative' if ci[1]<0 else 'observed_exact_zero' if all(x==0 for x in xs) else 'unresolved';estimates[k]=dict(mean=mean,ci95=ci,classification=cl,mean_percentage_points=mean*100,ci95_percentage_points=[x*100 for x in ci],expected_terminal_descendant_difference=mean*32 if k.endswith('|D40') else None,ci95_expected_descendants=[x*32 for x in ci] if k.endswith('|D40') else None,paired_signs=signs[k],block_signs=dict(collections.Counter(sign(x) for x in xs)))
 ...
 with (P/'ESTIMATES.csv').open('w') as f:
  w=csv.writer(f);w.writerow(['key','mean','ci_low','ci_high','mean_pp','low_pp','high_pp','classification','mean_descendant_difference','low_descendants','high_descendants'])
  for k,q in estimates.items():w.writerow([k,q['mean']]+q['ci95']+[q['mean_percentage_points']]+q['ci95_percentage_points']+[q['classification'],q['expected_terminal_descendant_difference']]+(q['ci95_expected_descendants'] or [None,None]))
 save('ASSESSMENT.json',dict(primary=PRIMARY,estimate=estimates[PRIMARY],prediction='positive',decisions=decisions(estimates[PRIMARY]['ci95']),interval_scope='approximate pointwise; no family-wide or conjunction coverage',interaction_direction_not_predicted=True,secondary_ham=estimates['HAM|Q_HALF_MINUS_ZERO|F_BG_EFFECT|D40'],secondary_interaction=estimates[CROSS+'|F_BG_EFFECT|D40'],half_founder_effect=estimates['TRAP4|Q_HALF|F_BG_EFFECT|D40'],half_absolute_introduced_H_change=estimates['TRAP4|Q_HALF|F_BG_SWITCH|D40'],half_population_effects={m:estimates['TRAP4|Q_HALF|F_BG_EFFECT_POP|'+m] for m in ('U','U40')},cohort_pooling=False))
 save('VALIDATION_CHECK.json',dict(status='PASS',new_paths=nnew,reused_paths=nold,source_prepared_states=768,raw_audit_block=0,raw_paths_audited=0,raw_audit_deferred_to='audit_independent.py',new_objective_calls=0,new_draws=0,intervals=114,structural_zeros=0,negative_paired_records=neg,transmission_accuracy_disagreements=disagreements,checks=counters,clock_tolerance_ulp=2,survivor_indices_and_order_exact=None,elapsed_seconds=time.time()-start))
