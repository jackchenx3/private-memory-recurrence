...
def main():
 ...
 with (P/'CONFIGURATION_METRICS.jsonl').open('w') as mout,(P/'TYPE_EVENTS.jsonl').open('w') as eout,(P/'STATE_DECLINES.jsonl').open('w') as dout:
  for block in range(24):
   with gzip.open(P/('raw/block%02d.jsonl.gz'%block),'rt') as src:
    for line in src:
     row=json.loads(line);rec=row['record'];g,c,cfg,rep=row['geometry'],row['cost'],row['configuration'],row['replicate'];cost=int(c[-1]);gp=g+'|'+c+'|'+cfg;ident=dict(block=block,replicate=rep,law=g,cost=c,configuration=cfg);freq,util=extract(rec,row['founder_ids']);assert freq==row['founder_frequencies'];pf=rec['frequencies'];rows[block,rep][g,c,cfg]=util;n+=1;pen=rec['penalized_accuracies'];mout.write(json.dumps(dict(ident,utilities=util,penalized_accuracy=dict(initial=pen[0],mean=sum(pen[1:])/40,terminal=pen[-1])))+'\n')
     ...
     h,f,r=(freq[k][-1] for k in ('p0','p1','R'));category='both_present' if h and f else 'p0only' if h else 'p1only' if f else 'both_absent';joint[gp][category]+=1;joint[gp][category+('_with_residents' if r else '_without_residents')]+=1;joint[gp]['p0_full_fixation']+=int(h==1);joint[gp]['p1_full_fixation']+=int(f==1);joint[gp]['carrier_fixation_with_both_lineages']+=int(h>0 and f>0 and r==0)
     for j,st in enumerate(rec['steps'],1):
      assert st['evaluator_calls']==128 and sum(st['current_descendant_counts'])==32;before=rec['populations'][j-1];after=rec['populations'][j];assert all(sum(pf[k][i] for k in ('H','F','R'))==1 for i in (j-1,j));car0=freq['CARR'][j-1];car1=freq['CARR'][j];assert st['S_pen']==st['S_raw']+cost*(car0-car1)/32
 ...
 assert n==4608 and len(rows)==192 and len(events)==96 and len(joint)==24
 ...
 save('FATE_GROUP_CHECK.json',dict(status='PASS',founder_groups=48,aggregate_carrier_groups=24,resident_fate_groups=24,joint_groups=24))
  ...
  for (b,r),data in rows.items():
   ...
   for g in LAWS:
    for c in COSTS:
     ...
     for ori in (0,1):
      ov=orientation(q,ori);orows[g,c,ori]=ov;oout.write(json.dumps(dict(block=b,replicate=r,law=g,cost=c,focal_position='p'+str(ori),historical_competitor_pair=['HH','MIX'+str(1-ori)],fresh_competitor_pair=['MIX'+str(ori),'FF'],values=ov))+'\n')
 ...
 assert len(counts)==len(blocksigns)==1716;save('INDIVIDUAL_SIGNS.json',counts);save('BLOCK_SIGNS.json',blocksigns);save('ORIENTATION_SIGNS.json',orient_counts)
 ...
 assert len(cross)==132;save('WINDOW_CROSSINGS.json',cross)
