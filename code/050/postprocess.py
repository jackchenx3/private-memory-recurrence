...
import pathlib,json,gzip,collections,csv,math
...
def arms(v,a,g,pol,ms):
 abundances=('RACE32','BEST32') if a=='RACE32_MINUS_BEST32' else (a,);regs=tuple(g.split('_MINUS_')) if '_MINUS_' in g else (g,);ps=pol.split('-');return {ab+'|'+r+'|'+p:{m:v[ab+'|'+r+'|'+p+'|'+m] for m in ms} for ab in abundances for r in regs for p in ps}
def main():
   ...
   with gzip.open(P/('raw/block%02d.jsonl.gz'%b),'rt') as src:
    for line in src:
     ...
     for j,s in enumerate(r['steps'],1):
      assert sum(s['current_descendant_counts'])==32 and s['frequency_term']==f[j]-f[j-1];assert s['persistent_cache_count']==32*f[j];assert all(s['candidates'][i][1:3]==r['populations'][j-1][i%32][1:3] for i in range(128));assert a=='RACE32' or s['S_pen']>=0;cost=int(pol.endswith('C1'));assert s['S_pen']==s['S_raw']+cost*(f[j-1]-f[j])/32
      if a=='RACE32':
       ks=s['survival_integers'];us=s['survival_uniforms'];scores=s['candidate_penalized_mismatches'];keys=[-math.log(u)*float(1<<v) for u,v in zip(us,scores)];assert len(keys)==128 and all(0<u<1 and u==(k+1)/(2**52+1) for k,u in zip(ks,us));assert keys==s['race_keys'] and all(math.isfinite(x) and x>0 for x in keys);assert s['selected_indices']==sorted(range(128),key=lambda i:(keys[i],s['ties'][i],i))[:32]
      ...
      if bad or s['S_raw']<0 or s['S_pen']<0:dout.write(json.dumps(dict(ident,update=j,declining_losses=bad,S_raw=s['S_raw'],S_pen=s['S_pen'],W_raw=s['W_raw'],W_pen=s['W_pen'],f_before=f[j-1],f_after=f[j],raw_loss=r['raw_losses'][j],penalized_loss=r['penalized_losses'][j]))+'\n');declines+=1
     for suffix,ls in [('raw',r['raw_losses']),('pen',r['penalized_losses'])]:
 ...
 assert n==6912 and len(groups)==192
 ...
 assert len(counts)==len(blocks)==2808
 ...
 bounds={g:dict(event_blocks=len(bs),block_event_indicators=[int(b in bs) for b in range(24)],independent_units=24,continuations_per_block=8,zero_event_bound_applicable=len(bs)==0,upper95=1-.05**(1/24.) if not bs else None,unit='probability a fresh eight-continuation block has any endpoint-surviving path',assumption='24 independent identically designed blocks',not_per_lineage_probability=True,not_mean_frequency_interval=True) for g,bs in surviving_blocks.items()};assert len(bounds)==36;save('RARE_BLOCK_EVENTS.json',bounds);save('ALL_PATH_EVENT_SUMMARIES.json',event_paths)
 ratios={g:dict(ratio=q['mean']/values[g[:-3]+'F0']['mean'],ci95=[x/values[g[:-3]+'F0']['mean'] for x in q['ci95']],same_evidence=True) for g,q in values.items() if g.endswith('|F40') and g.split('|')[0] in ('BEST32','RACE32') and g.split('|')[1] in ('ZERO','HALF','FULL') and '-' not in g.split('|')[2]};assert len(ratios)==36;save('ENDPOINT_INITIAL_RATIOS.json',ratios)
 ...
 old_bounds=json.loads((P.parent/'intermittent_private_memory_v1/RARE_BLOCK_EVENTS.json').read_text());compared=0
 for k,v in bounds.items():
  if k.startswith('BEST32|'):assert v==old_bounds[k.replace('BEST32|','RESIDENT40|')];compared+=1
 assert compared==18;save('BASELINE_EVENT_CHECK.json',dict(status='PASS',old_exact_block_event_records=18))
 save('POSTPROCESS_CHECKS.json',dict(status='PASS',new_paths=3456,old_paths=3456,records=2808,negative_records=negative,endpoint_disagreements=disagreements,decline_records=declines,max_weighted_telescoping_residual=maxerr,extra_objective_calls=0));print(json.dumps(json.loads((P/'POSTPROCESS_CHECKS.json').read_text())))
