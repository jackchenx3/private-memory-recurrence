...
def main():
  ...
  for block in range(24):
   with gzip.open(P/('raw/block%02d.jsonl.gz'%block),'rt') as src:
    for line in src:
     ...
     roles=('R','H','F') if cfg.startswith('MIX') else ('R',cfg[0])
     for role in roles:
     ...
     if cfg.startswith('MIX'):
      h,f,r=(freq[k][-1] for k in ('H','F','R'));category='both_present' if h and f else 'Honly' if h else 'Fonly' if f else 'both_absent';joint[gp][category]+=1;joint[gp][category+('_with_residents' if r else '_without_residents')]+=1;joint[gp]['H_full_fixation']+=int(h==1);joint[gp]['F_full_fixation']+=int(f==1)
     for j,st in enumerate(rec['steps'],1):
      assert st['evaluator_calls']==128 and sum(st['current_descendant_counts'])==32;before=rec['populations'][j-1];after=rec['populations'][j];assert all(sum(freq[k][i] for k in ('H','F','R'))==1 for i in (j-1,j));car0=freq['H'][j-1]+freq['F'][j-1];car1=freq['H'][j]+freq['F'][j];assert st['S_pen']==st['S_raw']+cost*(car0-car1)/32
      mapping={int(k):v for k,v in row['logical_roles'].items()}
      for role in ('H','F','R'):
       term=sum((d-1)*(int(mapping[q[1]]==role)-freq[role][j-1]) for d,q in zip(st['current_descendant_counts'],before))/32.;assert term==freq[role][j]-freq[role][j-1];assert freq[role][j-1]!=0 or freq[role][j]==0
 ...
 assert n==6912 and len(rows)==192 and len(events)==84 and len(joint)==12
 ...
 old=json.loads((P.parent/'probabilistic_private_memory_v1/RARE_BLOCK_EVENTS.json').read_text());checked=0
 for g in LAWS:
  for c in COSTS:
   for cfg,policy in [('H0','ACTIVE'),('F0','NOVEL')]:assert bounds[g+'|'+c+'|'+cfg+'|'+cfg[0]]==old['RACE32|'+g+'|'+policy+'_'+c];checked+=1
 assert checked==12;save('BASELINE_EVENT_CHECK.json',dict(status='PASS',exact_old_block_event_records=12,introduced_carrier_fate_groups=48,resident_fate_groups=36))
  ...
  for (b,r),data in rows.items():
   ...
   for g in LAWS:
    for c in COSTS:
     ...
     for ori,hc,fc in [(0,'H0','F1'),(1,'H1','F0')]:
      mix=q['MIX'+str(ori)];isoH=q[hc];isoF=q[fc];ov={}
      for metric in FREQ:
       mh,mf,ih,iff=(mix['frequency']['H'][metric],mix['frequency']['F'][metric],isoH['frequency']['H'][metric],isoF['frequency']['F'][metric]);ov.update({s+'|'+metric:x for s,x in [('MIX_H',mh),('MIX_F',mf),('ISO_H',ih),('ISO_F',iff),('MIX_H_MINUS_F',mh-mf),('ISO_H_MINUS_F',ih-iff),('MIX_MINUS_ISO_H',mh-ih),('MIX_MINUS_ISO_F',mf-iff),('MIX_MINUS_ISO_RANK',(mh-mf)-(ih-iff))]})
      for metric in RAW:
       mm,ih,iff=mix['raw'][metric],isoH['raw'][metric],isoF['raw'][metric];ov.update({s+'|'+metric:x for s,x in [('MIX_POP',mm),('ISO_H_POP',ih),('ISO_F_POP',iff),('MIX_MINUS_ISO_H_POP',mm-ih),('MIX_MINUS_ISO_F_POP',mm-iff),('ISO_H_MINUS_F_POP',ih-iff)]})
      orows[g,c,ori]=ov
      oout.write(json.dumps(dict(block=b,replicate=r,law=g,cost=c,orientation=ori,isolated_H=hc,isolated_F=fc,values=ov))+'\n')
      for k,x in ov.items():orient_counts[g+'|'+c+'|MIX'+str(ori)+'|'+k][sign(x)]+=1
   for g,h in LPAIRS:
    for c in COSTS:
     for ori in (0,1):
      ...
      for k,x in ov.items():orient_counts[g+'_MINUS_'+h+'|'+c+'|MIX'+str(ori)+'|'+k][sign(x)]+=1
   derive(v);pout.write(json.dumps(dict(block=b,replicate=r,values=v))+'\n')
   for k,x in v.items():
 ...
 assert len(counts)==len(blocksigns)==1482;save('INDIVIDUAL_SIGNS.json',counts);save('BLOCK_SIGNS.json',blocksigns);save('ORIENTATION_SIGNS.json',orient_counts)
 ...
 assert len(cross)==114;save('WINDOW_CROSSINGS.json',cross)
