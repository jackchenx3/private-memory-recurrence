...
def main():
 ...
 try:
  ...
  stages.append(stage('make_inputs.py','inputs.log',guard));stages.append(stage('transfer.py','transfer.log',guard));save('RUNTIME.json',dict(elapsed_seconds=time.time()-start,stages=stages,io_advice=ADVICE))
  files=['TIME_SERIES_IDENTITIES.jsonl.gz','SEED_ROSTER.json','PRIOR_SEED_INVENTORY.json','survival_streams.jsonl.gz','RESIDENT_STATES.jsonl.gz','RESIDENT_PROVENANCE.json','TARGET_LAW_DRAWS.json','ZERO_TARGETS.json','HALF_TARGETS.json','FULL_TARGETS.json','streams.jsonl.gz','INPUT_REFERENCES.json','SEEDS.json','SEED_COLLISION_CHECK.json','fresh_probe_masks.jsonl.gz','BOOTSTRAP_INDICES.json','INPUT_GENERATION.json','INPUT_SHA256SUMS','INPUT_FREEZE.json','inputs.log','RUN_STARTED.json','FIXTURE_WORK.json','tests.log','TEST_STATUS.json','TRANSFER_STARTED.json','TRANSFER_PROGRESS.json','BASE_BLOCKS.json','PREFIX_CHECKS.json','TRANSFER_RUNTIME.json','BUDGET.json','RUNTIME.json','transfer.log','PREPARATION_TARGETS.json']+['raw/block%02d.jsonl.gz'%i for i in range(24)];hashes=[]
