...
def main():
 ...
 assert len(values)==429 and sum(structural(k) for k in values)==21
 save('summary.json',dict(primary='HALF|C0|LOW_H|D40',values=values,interval_scope='All intervals including primary approximate pointwise;24blocks x8 paired continuations; orientations not independent replicates',numpy_version=np.__version__,python=sys.version));save('INFERENCE_CHECK.json',dict(status='PASS',records=429,structural_zero_records=21,new_objective_calls=0,quantile_method='linear'))
 print(values['HALF|C0|LOW_H|D40'])
