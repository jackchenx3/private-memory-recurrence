...
def main():
 ...
 try:
  ...
  files=['SEED_ROSTER.json','PRIOR_SEED_INVENTORY.json','survival_streams.jsonl.gz','RESIDENT_STATES.jsonl.gz','RESIDENT_PROVENANCE.json','TARGET_LAW_DRAWS.json','ZERO_TARGETS.json','HALF_TARGETS.json','FULL_TARGETS.json','streams.jsonl.gz','INPUT_REFERENCES.json','SEEDS.json','SEED_COLLISION_CHECK.json','fresh_probe_masks.jsonl.gz','BOOTSTRAP_INDICES.json','INPUT_GENERATION.json','INPUT_SHA256SUMS','INPUT_FREEZE.json','inputs.log','RUN_STARTED.json','FIXTURE_WORK.json','tests.log','TEST_STATUS.json','TRANSFER_STARTED.json','TRANSFER_PROGRESS.json','BASE_BLOCKS.json','PREFIX_CHECKS.json','TRANSFER_RUNTIME.json','BUDGET.json','RUNTIME.json','transfer.log','PREPARATION_TARGETS.json','prep_streams.jsonl.gz','PREPARATION_STARTED.json','PREPARATION_RUNTIME.json','PREPARATION_BUDGET.json','RESIDENT_SHA256SUMS','RESIDENT_FREEZE.json','preparation.log']+['raw/block%02d.jsonl.gz'%i for i in range(24)]+['preparation/block%02d.jsonl.gz'%i for i in range(24)];hashes=[]
