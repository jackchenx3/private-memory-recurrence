"""056 production allocation only; reuse accepted failure-propagating memory guard."""
...
def main():
 ...
 try:
  verify_manifest('SOURCE_SHA256SUMS');verify_manifest('INPUT_SHA256SUMS');assert read('TEST_STATUS.json')['status']=='PASS';guard.check()
  steps.append(stage('transfer.py','transfer.log',guard))
  ...
  steps.append(stage('validate_analyze.py','validation.log',guard));save('RUNTIME.json',dict(elapsed_seconds=time.time()-start,stages=steps));guard.check();success=True
 except BaseException as e:
  save('EXECUTION_FAILURE.json',dict(error=str(e),traceback=traceback.format_exc(),partial_outputs_preserved=True));raise
 ...
 save('COMPLETION.json',dict(status='COMPLETE_PENDING_ACCOUNTING',job_id=os.environ['SLURM_JOB_ID'],elapsed_seconds=time.time()-start,source_sha256=sha(P/'SOURCE_SHA256SUMS'),result_sha256=sha(P/'RESULT_SHA256SUMS'),validation='PASS',new_paths=2304,reused_paths=2304,intervals=162))
