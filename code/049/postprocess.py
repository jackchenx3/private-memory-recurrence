...
def arms(v,a,g,pol,ms):
 abundances=('RESIDENT40','NAIVE') if a=='RESIDENT40_MINUS_NAIVE' else (a,);regs=tuple(g.split('_MINUS_')) if '_MINUS_' in g else (g,);ps=pol.split('-');return {ab+'|'+r+'|'+p:{m:v[ab+'|'+r+'|'+p+'|'+m] for m in ms} for ab in abundances for r in regs for p in ps}
def main():
 ...
 assert n==3456 and len(groups)==192
 ...
 assert len(counts)==len(blocks)==1248
 ...
 bounds={g:dict(event_blocks=len(bs),block_event_indicators=[int(b in bs) for b in range(24)],independent_units=24,continuations_per_block=8,zero_event_bound_applicable=len(bs)==0,upper95=1-.05**(1/24.) if not bs else None,unit='probability a fresh eight-continuation block has any endpoint-surviving path',assumption='24 independent identically designed blocks',not_per_lineage_probability=True,not_mean_frequency_interval=True) for g,bs in surviving_blocks.items()};assert len(bounds)==18;save('RARE_BLOCK_EVENTS.json',bounds);save('ALL_PATH_EVENT_SUMMARIES.json',event_paths)
 ratios={g:dict(ratio=q['mean']/values[g[:-3]+'F0']['mean'],ci95=[x/values[g[:-3]+'F0']['mean'] for x in q['ci95']],same_evidence=True) for g,q in values.items() if g.endswith('|F40') and g.split('|')[0] in ('NAIVE','RESIDENT40') and g.split('|')[1] in ('ZERO','HALF','FULL') and '-' not in g.split('|')[2]};assert len(ratios)==18;save('ENDPOINT_INITIAL_RATIOS.json',ratios)
 ...
 old_bounds=json.loads((P.parent/'prepared_resident_introduction_v1/RARE_BLOCK_EVENTS.json').read_text());compared=0
 for k,v in bounds.items():
  if '|FULL|' in k:assert v==old_bounds[k.replace('|FULL|','|RECUR|')];compared+=1
 assert compared==6;save('BASELINE_EVENT_CHECK.json',dict(status='PASS',old_exact_block_event_records=6))
 save('POSTPROCESS_CHECKS.json',dict(status='PASS',new_paths=2304,old_paths=1152,records=1248,negative_records=negative,endpoint_disagreements=disagreements,decline_records=declines,max_weighted_telescoping_residual=maxerr,extra_objective_calls=0));print(json.dumps(json.loads((P/'POSTPROCESS_CHECKS.json').read_text())))
