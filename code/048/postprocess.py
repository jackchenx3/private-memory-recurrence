...
def main():
   ...
   with gzip.open(P/('raw/block%02d.jsonl.gz'%b),'rt') as src:
    for line in src:
     ...
     for j,s in enumerate(r['steps'],1):
      ...
      if pol.startswith('RADIUS'):
       q=radius_counts[gp][j-1];q['states']+=1;q['recipient_slots']+=32;q['extinct_states']+=int(f[j-1]==0);q['fixed_states']+=int(f[j-1]==1);q['polymorphic_states']+=int(0<f[j-1]<1)
       for i,z in enumerate(s['radius_probe']):
        parent=r['populations'][j-1][i];used=bool(parent[1] and parent[3] is not None);d=bin(parent[0]^parent[3][0]).count('1') if used else 0
        assert sorted(z['permutation'])==list(range(32)) and z['distance']==d and z['use']==used;mask=sum(1<<bit for bit in z['permutation'][:d]);assert z['mask']==mask and z['expressed_genotype']==parent[0]^mask==s['candidates'][32+i][0]
        q['used' if used else 'unused']+=1;q['all_slots_distance_'+str(d)]+=1
        if used:q['used_distance_'+str(d)]+=1
 ...
 assert n==6144 and len(groups)==192
 ...
 assert len(counts)==len(blocks)==1664
 ...
 bounds={g:dict(event_blocks=len(bs),block_event_indicators=[int(b in bs) for b in range(24)],independent_units=24,continuations_per_block=8,zero_event_bound_applicable=len(bs)==0,upper95=1-.05**(1/24.) if not bs else None,unit='probability a fresh eight-continuation block has any endpoint-surviving path',assumption='24 independent identically designed blocks',not_per_lineage_probability=True,not_mean_frequency_interval=True) for g,bs in surviving_blocks.items()};assert len(bounds)==32;save('RARE_BLOCK_EVENTS.json',bounds);save('ALL_PATH_EVENT_SUMMARIES.json',event_paths)
 ratios={g:dict(ratio=q['mean']/values[g[:-3]+'F0']['mean'],ci95=[x/values[g[:-3]+'F0']['mean'] for x in q['ci95']],same_evidence=True) for g,q in values.items() if g.endswith('|F40') and g.split('|')[0] in ('NAIVE','RESIDENT40') and g.split('|')[1] in ('IID','RECUR') and '-' not in g.split('|')[2]};assert len(ratios)==32;save('ENDPOINT_INITIAL_RATIOS.json',ratios)
 ...
 save('RADIUS_PROBE_COUNTS.json',dict(per_update=radius_counts,per_group={g:dict(sum(rows,collections.Counter())) for g,rows in radius_counts.items()},denominators='Each RADIUSgroup192paths x40updates x32slots; unused/extinct and d0/d32 retained; distances computed on own visited state, not equal cross-arm sequences.'))
 old_bounds=json.loads((P.parent/'prepared_resident_introduction_v1/RARE_BLOCK_EVENTS.json').read_text());assert len(old_bounds)==24
 for k,v in old_bounds.items():assert bounds[k]==v,k
 save('BASELINE_EVENT_CHECK.json',dict(status='PASS',old_exact_block_event_records=24))
 save('POSTPROCESS_CHECKS.json',dict(status='PASS',new_paths=1536,old_paths=4608,records=1664,negative_records=negative,endpoint_disagreements=disagreements,decline_records=declines,max_weighted_telescoping_residual=maxerr,extra_objective_calls=0));print(json.dumps(json.loads((P/'POSTPROCESS_CHECKS.json').read_text())))
