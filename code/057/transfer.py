"""All4,608 new057 crossed transfers; no old states, trajectories or tapes."""
...
def main():
  ...
  for b in range(24):
   with File(P/('raw/block%02d.jsonl.gz'%b),'w') as out:
    for r in range(8):
     ...
     for p,q in [('ZERO','ZERO'),('ZERO','HALF'),('HALF','ZERO'),('HALF','HALF')]:
      ...
      for bg in ('F','H'):
       ...
       for var in ('STAY','SWITCH0','SWITCH1'):
        ...
        out.write(dict(block=b,replicate=r,preparation=p,future=q,cell=cell,configuration=cfg,background=bg,cost='C0',founder_ids=labels[:2],rare_founder_id=slot,preparation_founder_map=prov,role_frequencies=freq,record=rec,evidence_kind='new057 independent-cohort transfer'))
 ...
 save('BUDGET.json',dict(new_paths=n,reused_paths=0,initial_preparation_paths=192,policy_preparation_paths=768,new_seed_records=3193,new_updates=n*40,candidate_evaluations=n*5120,initial_evaluations=n*32,transfer_objective_calls=n*5152,total_scientific_paths=5568,total_updates=222720,total_candidate_evaluations=28508160,total_initial_evaluations=178176,total_state_measurements=228288,total_objective_calls=28686336,new_state_measurements=n*41,source_prepared_states_used=len(used),nonparent_sources=ops,extra_diagnostic_calls=0))
