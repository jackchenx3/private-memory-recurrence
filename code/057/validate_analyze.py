"""057 all-new saved-output aggregation; prospective block0 raw audit; no old cohort inputs."""
...
from accepted_validate056 import validate
...
def main():
  ...
  for b in range(24):
   ...
   for kind,filename in [('new',P/('raw/block%02d.jsonl.gz'%b))]:
    for original in rows(filename):
     ...
     if b==0:validate(row,st,streams,fresh,surv,table,counters)
 ...
 assert nnew==4608 and nold==0 and len(data)==192 and counters['clock_checks']==983040
 ...
 save('ASSESSMENT.json',dict(primary=PRIMARY,estimate=estimates[PRIMARY],prediction='positive',supported=estimates[PRIMARY]['classification']=='positive',contradicted=estimates[PRIMARY]['classification']=='negative',interval_scope='approximate pointwise',interaction_direction_not_predicted=True,prospective_within_model_replication=True,independent_replication=True,secondary='Q_HALF_MINUS_ZERO_AT_P_ZERO|F_BG_EFFECT|D40',secondary_estimate=estimates['Q_HALF_MINUS_ZERO_AT_P_ZERO|F_BG_EFFECT|D40'],secondary_expected_sign='positive',secondary_supported=estimates['Q_HALF_MINUS_ZERO_AT_P_ZERO|F_BG_EFFECT|D40']['classification']=='positive',cohort_pooling=False))
 save('VALIDATION_CHECK.json',dict(status='PASS',new_paths=nnew,reused_paths=nold,source_prepared_states=768,raw_audit_block=0,raw_paths_audited=192,new_objective_calls=0,new_draws=0,intervals=162,structural_zeros=0,negative_paired_records=neg,transmission_accuracy_disagreements=disagreements,checks=counters,clock_tolerance_ulp=2,survivor_indices_and_order_exact=True,elapsed_seconds=time.time()-start))
