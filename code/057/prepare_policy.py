"""Exact055 policy stages, now from new057 cohort and independent stage tapes."""
 ...
 with File(P/'INITIAL_STATES.jsonl.gz','r') as roots,File(P/'policy_streams.jsonl.gz','r') as streams,File(P/'policy_fresh_probe_masks.jsonl.gz','r') as fresh,File(P/'policy_survival_streams.jsonl.gz','r') as survival,File(P/'RESIDENT_STATES.jsonl.gz','w') as states:
  for b in range(24):
   ...
   with File(P/('policy/block%02d.jsonl.gz'%b),'w') as out:
    for r in range(8):
     root,d,f,s=next(roots),next(streams),next(fresh),next(survival);assert all((x['block'],x['replicate'])==(b,r) for x in (root,d,f,s))
     for p in ('ZERO','HALF'):
      for bg in ('F','H'):
       targets=ts[p][b]['targets'];rec=stage(targets,d['draws'],all_policy(root['genotypes'],bg),0,f['fresh'],s['survival']);pop,mapping=rebase(rec['populations'][-1]);assert rec['query_counts']['total']==5152
       out.write(dict(block=b,replicate=r,geometry=p,background=bg,record=rec));state=dict(block=b,replicate=r,geometry=p,background=bg,population=rec['populations'][-1],rebased_population=pop,founder_rebase_map=mapping,last_preparation_target=targets[-1],last_two_preparation_targets=targets[-2:],source='new057 unfiltered policy preparation');states.write(state);blockstates.append(state);n+=1
