"""Generate all independent057 exogenous tapes inside allocation before any trajectory."""
import os,random
from common import *
...
def main():
 ...
 histories={p:[] for p in ('ZERO','HALF')};targets={};initial=[];lawdraws=[];cdraws=[]
 for b in range(24):
  ...
  for p in histories:
   ...
   for q in histories:
    ts=extend_targets(hs,ci,cc,q);targets.setdefault('P_'+p+'_Q_'+q,[]).append(dict(block=b,targets=ts,preparation_terminal_targets=hs[-2:],boundary_previous_target=hs[-1],boundary_actual_change=hs[-1]!=ts[0]))
