...
def main():
  ...
  for block in range(24):
   with gzip.open(P/('raw/block%02d.jsonl.gz'%block),'rt') as src:
    for line in src:
     ...
     roles=('H','F','RARE') if 'SWITCH' in cfg else ('p0','p1')
     for role in roles:
     ...
     if 'SWITCH' in cfg:
      ...
      h,f=freq['p0'][-1],freq['p1'][-1];category='both_present' if h and f else 'p0only' if h else 'p1only' if f else 'neither';joint[gp][category]+=1;joint[gp]['p0_fixed']+=int(h==1);joint[gp]['p1_fixed']+=int(f==1);joint[gp]['other_founders_present']+=int(h+f<1);jout.write(json.dumps(dict(ident,category=category,surviving_founders=sorted(set(q[2] for q in rec['populations'][-1])),resident_policy_frequency=freq[cfg[0]][-1]))+'\n')
     for j,st in enumerate(rec['steps'],1):
      ...
      for role,fid in zip(('p0','p1'),row['founder_ids']):
      ...
      for i,q in enumerate(st['candidates']):assert q[1:3]==before[i%32][1:3];assert q[3]==(before[i][3] if i<32 else ([before[i%32][0],40+j-1,i%32] if before[i%32][1] else None));assert st['candidate_penalized_mismatches'][i]==st['candidate_raw_mismatches'][i]+cost*int(q[1]!=0)
      if cfg in NEW:
 ...
 assert n==3456 and len(rows)==192 and len(events)==48 and len(joint)==18
 save('TRANSFER_BOUNDARY_CHECK.json',dict(status='PASS',transfer_initial_states=3456,source_prepared_states=1152,cache_and_genotypes_preserved=True,founder_rebase_exact=True,absolute_updates=list(range(40,80)),new_fresh_arrays_matched=True,extra_objective_calls=0));save('TRAJECTORY_MEANS.json',traj);save('TYPE_EVENT_COUNTS.json',events);save('JOINT_ENDPOINT_COUNTS.json',joint);save('SWITCHED_JOINT_ENDPOINT_COUNTS.json',{k:v for k,v in joint.items() if 'SWITCH' in k});save('STAY_JOINT_ENDPOINT_COUNTS.json',{k:v for k,v in joint.items() if 'STAY' in k});save('SELECTION_DIAGNOSTICS.json',diagnostics);save('OBSERVED_ACCOUNTING.json',account);save('OPERATION_COUNTS.json',ops);bounds={k:bound(b) for k,b in bs.items()};save('ROLE_BLOCK_EVENTS.json',bounds)
 save('FATE_GROUP_CHECK.json',dict(status='PASS',switched_fate_groups=36,stay_founder_groups=12,switched_joint_groups=12,stay_joint_groups=6))
  ...
  for (b,r),data in rows.items():
   ...
   for g in LAWS:
    for c in COSTS:
     ...
     for ori in (0,1):
      ov=orientation(q,ori);orows[g,c,ori]=ov;oout.write(json.dumps(dict(block=b,replicate=r,law=g,cost=c,focal_position='p'+str(ori),matched_background_pairs=[['F_SWITCH'+str(ori),'F_STAY'],['H_SWITCH'+str(ori),'H_STAY']],values=ov))+'\n')
 ...
 assert len(counts)==len(blocksigns)==624;save('INDIVIDUAL_SIGNS.json',counts);save('BLOCK_SIGNS.json',blocksigns);save('ORIENTATION_SIGNS.json',orient_counts)
 ...
 assert len(cross)==48;save('WINDOW_CROSSINGS.json',cross)
