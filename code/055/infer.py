...
def main():
 ...
 assert set(values)==set(json.loads((P/'DESIGN_GRID.json').read_text())['keys'])
 assert len(values)==624 and sum(structural(k) for k in values)==24
 save('summary.json',dict(primary='HALF|C0|F_BG_EFFECT|D40',values=values,interval_scope='All intervals including primary approximate pointwise;24blocks x8 paired continuations; orientations not independent replicates',numpy_version=np.__version__,python=sys.version));save('INFERENCE_CHECK.json',dict(status='PASS',records=624,structural_zero_records=24,new_objective_calls=0,quantile_method='linear'))
 print(values['HALF|C0|F_BG_EFFECT|D40'])
