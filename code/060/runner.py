"""060 production allocation only; reuse accepted failure-propagating memory guard."""
...
def main():
 ...
 try:
  ...
  for script,log in [('make_inputs.py','inputs.log'),('transfer.py','transfer.log')]:steps.append(stage(script,log,guard))
  files=['TRANSFER_STARTED.json','PROGRESS.json','TIME_SERIES.jsonl.gz','PAIRED_METRICS.jsonl.gz','BUDGET.json','TRANSFER_RUNTIME.json','INPUT_FREEZE.json']+['raw/block%02d.jsonl.gz'%b for b in range(24)]
 ...
 save('COMPLETION.json',dict(status='COMPLETE_PENDING_ACCOUNTING',job_id=os.environ['SLURM_JOB_ID'],elapsed_seconds=time.time()-start,source_sha256=sha(P/'SOURCE_SHA256SUMS'),result_sha256=sha(P/'RESULT_SHA256SUMS'),validation='PASS',new_paths=2304,reused_preparations=192,new_preparations=0,total_scientific_paths=2304,intervals=81))
