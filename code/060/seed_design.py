import hashlib
PREFIX='ORG-OBSERVATION-060-r1'
BLOCK_FAMILIES=('continuation_innovations','continuation_copy')
FAMILIES=tuple('transfer_'+k for k in ('local','scout','ties','labels','fresh','survival','noise'))
...
def verify_roster(rs,prior):
 vals=[q['seed'] for q in rs];assert rs==roster() and len(vals)==len(set(vals))==1393 and not set(vals)&set(prior);return True
