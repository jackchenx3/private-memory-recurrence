...
ABUNDANCES=('NAIVE','RESIDENT40');ACCESS=('ACTIVE_C0-NOVEL_C0','ACTIVE_C1-NOVEL_C1')
...
keys=['RESIDENT40|RECUR|ACTIVE_C0-NOVEL_C0|D40','RESIDENT40|RECUR|ACTIVE_C0|D40','RESIDENT40|RECUR|ACTIVE_C0-SHAM_C0|D40','NAIVE|RECUR|ACTIVE_C0-NOVEL_C0|D40','RESIDENT40_MINUS_NAIVE|RECUR|ACTIVE_C0-NOVEL_C0|D40','RESIDENT40|IID|ACTIVE_C0-NOVEL_C0|D40','RESIDENT40|RECUR|ACTIVE_C1|D40'];fig,ax=plt.subplots(figsize=(16,7),constrained_layout=True)
for i,k in enumerate(keys):point(ax,k,i)
ax.set_yticks(range(len(keys)),keys);ax.axvline(0,color='.6');ax.set_xlabel('Carrier change or retrieval contrast');ax.set_title('Resident-state primary, separate spread and moderation; approximate pointwise intervals');finish(fig,'02_primary')
...
for row,g in enumerate(LAWS):
 for col,m in enumerate(('D40','U','U40')):
  ax=axes[row,col]
  for i,pol in enumerate(('ACTIVE_C0','ACTIVE_C0-NOVEL_C0','ACTIVE_C0-SHAM_C0')):point(ax,'RESIDENT40_MINUS_NAIVE|'+g+'|'+pol+'|'+m,i)
  ax.set_yticks(range(3),['History absolute','History minus fresh','History minus sham']);ax.set_ylim(-.4,2.4);ax.axvline(0,color='.6');ax.set_title(g+' '+m);ax.set_xlabel('Prepared minus naive: total effect')
