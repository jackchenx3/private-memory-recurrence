...
def main():
 ...
 try:
  ...
  files=['RESIDENT_STATES.jsonl.gz','RESIDENT_PROVENANCE.json','TARGETS.json','IID_TARGETS.json','RECUR_TARGETS.json','streams.jsonl.gz','NEUTRAL_FOUNDER_COUNTS.jsonl.gz','NEUTRAL_FOUNDER_CHECK.json','INPUT_REFERENCES.json','SEEDS.json','SEED_COLLISION_CHECK.json','fresh_probe_masks.jsonl.gz','BOOTSTRAP_INDICES.json','INPUT_GENERATION.json','INPUT_SHA256SUMS','INPUT_FREEZE.json','inputs.log','RUN_STARTED.json','FIXTURE_WORK.json','tests.log','TEST_STATUS.json','TRANSFER_STARTED.json','TRANSFER_PROGRESS.json','BASE_BLOCKS.json','PREFIX_CHECKS.json','TRANSFER_RUNTIME.json','BUDGET.json','RUNTIME.json','transfer.log']+['raw/block%02d.jsonl.gz'%i for i in range(24)];hashes=[]
