...
from analysis import MODES,NEW,STARTS
...
class Tests(unittest.TestCase):
 ...
 def seq(self,targets,mode,genotypes=None):
  rec=sequence(targets,self.draws,mode,self.fresh,[i*3 for i in range(32)] if genotypes is None else genotypes);Tests.calls+=rec['query_counts']['total'];Tests.sequences+=1;return rec
 ...
 def test_queries_zero_multiple_flips_and_donor_coordinates(self):
  pop=initial('ACTIVE_C0',self.labels,[0]*32);local=[0,3,0xffffffff]+[1<<i for i in range(3,32)];s=self.once(pop,'NOVEL_C0',15,local=local,scout=[0xffffffff]*32);self.assertEqual(s['evaluator_calls'],128);self.assertEqual([q['candidate_index'] for q in s['query_records']],list(range(128)))
 ...
 def test_xor_bijection_and_seed_draw_order(self):
  ...
  import hashlib
  self.assertEqual(seed('fresh_probe',3,7),int(hashlib.sha256(b'ORG-RESIDENT-047-r1|fresh_probe|3|7').hexdigest()[:16],16))
  class Fake:
 ...
 def test_grid_1716_98_exact_keys(self):
  v={a+'|'+g+'|'+p+'|'+m:0. for a in STARTS for g in ('IID','RECUR') for p in MODES for m in MET};derive(v);keys={k for k in v if structural(k)};self.assertEqual(len(v),1716);self.assertEqual(len(keys),98);self.assertEqual(keys,set(json.loads(pathlib.Path('DESIGN_CHECK.json').read_text())['structural_zero_keys']));self.assertFalse(structural('RESIDENT40|RECUR_MINUS_IID|NOVEL_C0|U0'));self.assertFalse(structural('RESIDENT40|RECUR_MINUS_IID|ACTIVE_C0-NOVEL_C0|F1'))
 ...
 def test_all_policy_dispatch_first_step_and_neutral(self):
  ...
  for cost in (0,1):
   a=self.seq([3,7]*20,'ACTIVE_C'+str(cost),genotypes=[0]*32);b=self.seq([3,7]*20,'SHAM_C'+str(cost),genotypes=[0]*32);self.assertEqual(first(a),first(b))
 ...
 def test_new_roster_and_draw_coordinates(self):
  ...
  iid,recur=target_rows(0,[5,5]+list(range(38)),list(range(40)),[9]*40);self.assertNotEqual(iid['targets'][:2],recur['targets'][:2]);self.assertEqual(recur['targets'],[9]*40);self.assertEqual(recur['actual_changes'],[False]*40);self.assertFalse(recur['boundary_change_from_preparation']);self.assertTrue(iid['boundary_change_from_preparation'])
 def test_nonzero_initialization_and_uniform_founder_positions(self):
  xs=[i*17 for i in range(32)];positions=[]
  for i in range(32):
   labels=list(range(i,32))+list(range(i));pop=initial('ACTIVE_C1',labels,xs);positions.append(next(j for j,p in enumerate(pop) if p[1]));self.assertEqual([p[0] for p in pop],xs);self.assertEqual([p[2] for p in pop],list(range(32)));self.assertEqual(pop[i][3],(xs[i],0,i));self.assertEqual(sum(p[1] for p in pop),1)
  self.assertEqual(positions,list(range(32)))
  from signatures import first
  a=self.seq([3,7]*20,'ACTIVE_C0',genotypes=xs);b=self.seq([3,7]*20,'SHAM_C0',genotypes=xs);self.assertEqual(first(a),first(b));self.assertEqual(founder_counts(b,self.labels)[0],[1]*32)
 def test_zero_genotype_novel_matches_accepted_and_frozen_assessment(self):
  import accepted_fresh_model
  for mode in ('NOVEL_C0','NOVEL_C1'):
   rec=self.seq([3,7]*20,mode,genotypes=[0]*32);ref=accepted_fresh_model.sequence([3,7]*20,self.draws,mode,self.fresh);Tests.calls+=ref['query_counts']['total'];Tests.sequences+=1;self.assertEqual(rec,ref)
  from assess_primary import assess
  v={k:dict(zero_classification='unresolved') for k in ('RESIDENT40|RECUR|ACTIVE_C0-NOVEL_C0|D40','RESIDENT40|RECUR|ACTIVE_C0|D40','RESIDENT40|RECUR|ACTIVE_C0-SHAM_C0|D40','RESIDENT40_MINUS_NAIVE|RECUR|ACTIVE_C0-NOVEL_C0|D40')};v['RESIDENT40|RECUR|ACTIVE_C0-NOVEL_C0|D40']['zero_classification']='positive';r=assess(v);self.assertTrue(r['historical_advantage_supported']);self.assertFalse(r['mean_spread_supported']);self.assertFalse(r['retrieval_attributed_spread_supported'])
