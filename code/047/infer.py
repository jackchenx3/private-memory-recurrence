...
def main():
 ...
 assert len(values)==1716 and sum(structural(k) for k in values)==98
 (P/'summary.json').write_text(json.dumps(dict(primary='RESIDENT40|RECUR|ACTIVE_C0-NOVEL_C0|D40',values=values,interval_scope='Paired046resident-state challenge;4608newpaths985newseeds;new2000wholeblockbootstrap;all intervals including primary approximate pointwise;no pooling',numpy_version=np.__version__,python=sys.version),indent=2)+'\n')
 (P/'INFERENCE_CHECK.json').write_text(json.dumps(dict(status='PASS',records=1716,structural_zero_records=98,numpy_version=np.__version__,quantile_method='linear',analysis_location='local postallocation; stored outputs only',new_dynamics=0),indent=2)+'\n')
 print(values['RESIDENT40|RECUR|ACTIVE_C0-NOVEL_C0|D40'])
