...
def inline(text,source):
    ...
    def link(match):
        ...
        if not url.startswith(('https://','http://')):
            ...
            url='https://github.com/jackchenx3/private-memory-recurrence/blob/v1.4.0/'+relative.as_posix()
...
def footer(canvas,doc):
    ...
    canvas.drawString(50,25,'Private memory and recurrence | Research preprint v1.4.0')
...
def render(source,dest):
    ...
    while i<len(lines):
        ...
        if line.startswith('|'):
            ...
            if cols==2 and rows[0][0].getPlainText()=='TRAP4 founder quantity':
                widths=[WIDTH*.55,WIDTH*.45]
