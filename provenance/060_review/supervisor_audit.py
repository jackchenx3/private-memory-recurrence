...
counts=collections.Counter();rawkeys=set();declines=0;race_max_ulps=0;race_max_relative=0;race_different_keys=0
for path in rows('raw/block00.jsonl.gz'):
 ...
 for g,step in enumerate(rec['steps']):
  ...
  keys=[-math.log(u)*(2.0**v) for u,v in zip(us,obs)]
  for actual,stored in zip(keys,step['race_keys']):
   ulps=abs(actual-stored)/max(math.ulp(actual),math.ulp(stored));assert ulps<=4,(actual,stored,ulps)
   race_max_ulps=max(race_max_ulps,ulps);race_max_relative=max(race_max_relative,abs(actual-stored)/max(actual,stored));race_different_keys+=int(actual!=stored)
  chosen=sorted(range(128),key=lambda j:(keys[j],ties[j],j))[:32];assert chosen==step['selected_indices'];after=[pool[j] for j in chosen];assert after==step['selected_records']==rec['populations'][g+1]
...
result={'task_id':'ORG-OBSERVATION-060','status':'PASS','checked_at_utc':datetime.now(timezone.utc).isoformat(),'producer_imports':False,'scientific_trajectories_rerun':0,'estimates_recomputed':81,'paired_metric_records':192*81,'maximum_estimate_discrepancy':maxerr,'raw_block':0,'counts':dict(counts),'retained_declines_in_audited_block':declines,'race_key_numerics':{'different_keys':race_different_keys,'maximum_ulp_difference':race_max_ulps,'maximum_relative_difference':race_max_relative,'allowed_ulp_bound':4,'all_selected_orders_exactly_identical':True,'note':'Initial bitwise equality failed between HPC and local math results. Small numerical differences are recorded; survivor choices and all subsequent states must still be identical.'},'primary':checked['NOISE_BY_RECURRENCE|EFFECT|D40'],'recurrence':{a:checked[a+'_Q_HALF_MINUS_ZERO|EFFECT|D40'] for a in ['EXACT','NOISY']},'script_sha256':sha(Path(__file__)),'source_manifest_sha256':sha(P/'SOURCE_SHA256SUMS'),'result_manifest_sha256':sha(P/'RESULT_SHA256SUMS'),'files_bound':{n:sha(P/n) for n in ['ESTIMATES.json','TIME_SERIES.jsonl.gz','PAIRED_METRICS.jsonl.gz','BOOTSTRAP_INDICES.json','raw/block00.jsonl.gz']}}
